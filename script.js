document.addEventListener("DOMContentLoaded", () => {
    ...
    const filtrarEMostrarProdutos = () => {
        ...
        if (produtosFiltrados.length === 0) {
            ...
            container.innerHTML = produtosFiltrados
                .map(
                    (p, index) => `
                        <div class="product-card" data-id="${p.id}" style="animation-delay: ${index * 0.1}s">
            ...
            // Aplicar animação de entrada para produtos
    ...
    const finalizarPedido = () => {
        ...
        const numeroWhatsApp = "5519987743878";
    ...
    // Animação de entrada para elementos quando eles aparecem na viewport
    ...
    document.querySelectorAll('.product-card, .category-btn').forEach(el => {
        observer.observe(el);
    });

    // Efeito de digitação para o texto hero
